...
public:
    ...
    uint32_t set_target_position(double degree, int velocity_val = 0) { // [/3200 = round] - Comment likely referring to a rounding aspect in the underlying implementation
        tgt_degree = degree;
        if (degree < 0) {
            degree = -degree;
            dir = set_dir ? 0 : 1; // Reverse direction if the target degree is negative
        } else {
            dir = set_dir;
        }
        ...
        if (velocity_val) {
            set_velocity(velocity_val); // [RPM] - Set velocity if a velocity value is provided
        }
